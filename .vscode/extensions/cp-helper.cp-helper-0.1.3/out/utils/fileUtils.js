...
async function pickFolder(currentUri) {
    ...
    let ignoreList = [];
    const ignoreEntry = items.find(([name, fileType]) => name === '.cphignore' && (fileType & vscode.FileType.File));
    if (ignoreEntry) {
...
async function checkFileDoesNotExist(fileUri, fileDisplayName) {
    try {
